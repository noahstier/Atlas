...
class VoxelNet(pl.LightningModule):
    ...
    def backproject(self, voxel_dim, voxel_size, origin, projection, features, pose):
        ...
        for b in range(batch):
            ...
            view_volume[b,:,valid[b]] = view_feats[b, :, valid[b]]

        volume = volume.view(batch, channels, nx, ny, nz)
        view_volume = view_volume.view(batch, view_feats.shape[1], nx, ny, nz)
        valid = valid.view(batch, 1, nx, ny, nz)

        volume = torch.cat((volume, view_volume), dim=1)
    
        return volume, valid, world

    def inference1(self, projection, pose, image=None, feature=None):
        ...
        volume, valid, world = self.backproject(voxel_dim, self.voxel_size, self.origin,
                                    projection, feature, pose)
    ...
    def forward(self, batch):
        ...
        if (not self.batch_backbone2d_time) or (not self.training) or self.finetune3d:
            ...
            for image, projection, pose in zip(images, projections, poses):
                self.inference1(projection, pose, image=image)
            ...
            for feature, projection, pose in zip(features, projections, poses):
                self.inference1(projection, pose, feature=feature)
